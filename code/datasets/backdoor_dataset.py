...
class SurrogateDataset(Dataset):
    ...
    def __init__(self, img_dir, victim_feature_bank_dir, transform, view_transform, k):
        ...
        try:
            self.targets = self.img_npz['y'][:,0].tolist()
        except:
            self.targets = self.img_npz['y']
...
class CIFAR10PairBD(Dataset):
    """CIFAR10 Dataset.
    """

    def __init__(self, numpy_file, trigger_file,target_file, indices, class_type, transform=None, bd_transform=None):
        """
        Args:
        ...
        self.target_input_array = np.load(target_file)
    ...
    def __getitem__(self, index):
        ...
        img_copy[:, : ,:] =img_copy * self.trigger_mask_list[0] + self.trigger_patch_list[0][:]
        img_backdoor =self.bd_transform(Image.fromarray(img_copy))

        target_img = Image.fromarray(self.target_image_list[0])
        target_image = self.bd_transform(target_img)
        target_img_1 = self.transform(target_img)

        return im_1, img_raw, img_backdoor, target_image, target_img_1
...
class CIFAR10TestBackdoor(Dataset):
    """cifar10 dataset."""

    def __init__(self, numpy_file, trigger_file, target_file, class_type, transform=None):
        """
        Args:
            numpy_file (string): Path to the numpy file.
            transform (callable, optional): Optional transform to be applied
        ...
        self.targets = [self.target_input_array['y'][0]]*self.data.shape[0]
    ...
    def __getitem__(self,index):
        ...
        img_backdoor =self.bd_transform(Image.fromarray(img))
...
class CIFAR10TargetImage(Dataset):

    def __init__(self, target_file, transform=None):
        """
        Args:
            numpy_file (string): Path to the numpy file.
            transform (callable, optional): Optional transform to be applied
...
######### multiple target images and trigger files

class CIFAR10PairBDMulti(Dataset):
    """CIFAR10 Dataset.
    """

    def __init__(self, numpy_file, trigger_file, target_file, indices, class_type, transform=None, bd_transform=None, ftt_transform=None):
        """
        Args:
            numpy_file (string): Path to the numpy file.
            transform (callable, optional): Optional transform to be applied
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.target_image_list)):
            img_copy[:, : ,:] =img_copy * self.trigger_mask_list[i] + self.trigger_patch_list[i][:]
            img_backdoor =self.bd_transform(Image.fromarray(img_copy))
...
class CIFAR10PairBDFTT(Dataset):
    """CIFAR10 Dataset.
    """
